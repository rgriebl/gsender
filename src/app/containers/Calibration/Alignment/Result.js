...
import {
    ...
    calculateFM,
    calculateMovement,
    FM_LOWER_OFFSET_THRESHOLD,
    RESULT_OFFSET_THRESHOLD,
} from './utils/result';
...
const Result = ({ triangle, jogValues, onBack, onClose }) => {
    ...
    const handleUpdateClick = () => {
        ...
        Confirm({
            ...
            content: (
                <>
                    <p>
                        Your machines EEPROM settings will be updated to the
                        following:
                    </p>
                    {x?.should && (
                        <p>
                            <strong>$100</strong> - X steps/mm will be updated
                            to <strong>{x?.amount}</strong>
                        </p>
                    )}
                    {y?.should && (
                        <p>
                            <strong>$101</strong> - Y steps/mm will be updated
                            to <strong>{y?.amount}</strong>
                        </p>
                    )}
                </>
            ),
                ...
                <>
                    ...
                    >
                        ...
                        <p className={styles.resultText}>
                            There was an error in the calculation, please check
            ...
            return (
                <>
                    ...
                    >
                        <Icon type="success" />
                        <p className={styles.resultText}>
                            Your machine is properly squared, no adjustments are
                            needed!
                        </p>
                    </div>

                    <FunctionButton onClick={onClose}>
            ...
            return (
                <>
                    ...
                    >
                        <Icon type="warning" />
                        <p>
                            Your machine is pretty squared, but if you still
                            want to have it full squared, move either the right
                            y-axis rail backwards <strong>{fm}mm</strong> or the
                            left y-axis rail forward by <strong>{fm}mm</strong>
                        </p>

                        {stepsUpdate && (
                            <p className={styles.resultText}>
                                We also noticed your steps-per-mm can be updated
                                for improved accuracy on your machine, you can
                                click &quot;Update Machine Settings&quot; to
                                update your EEPROM values.
                            </p>
                        )}
                    </div>
        ...
        return (
            <>
                <div className={classnames(styles.result, styles.resultError)}>
                    <Icon type="error" />
                    <p>
                        Your machine is off by <strong>{fm}mm</strong>, to
                        properly square it, move either the right y-axis rail{' '}
                        {rightDirection} <strong>{fm}mm</strong> or the left
                        y-axis rail {leftDirection} by <strong>{fm}mm</strong>
                    </p>

                    {stepsUpdate && (
                        <p>
                            We also noticed your steps-per-mm can be updated for
                            improved accuracy on your machine, you can click
                            &quot;Update Machine Settings&quot; to update your
                            EEPROM values.
                        </p>
                    )}
                    ...
                    {fm > RESULT_OFFSET_THRESHOLD && (
                        ...
                        >
                            Warning. Your machine is off by a large amount,
                            updating the EEPROM values for improved accuracy may
                            cause issues.
                        </p>
                    )}
                </div>
    ...
    return <div className={styles.resultPageWrapper}>{renderResult()}</div>;
};
